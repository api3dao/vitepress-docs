...
[README](https://github.com/api3dao/airnode/tree/v0.14/packages/airnode-deployer/docker)
...
-e USER_ID=$(id -u) -e GROUP_ID=$(id -g) \
-v "$(pwd):/app/config" \
api3/airnode-deployer:0.15.0 deploy --auto-remove false
```
...
docker run -it --rm \
  -v "$(pwd):/app/config" \
  api3/airnode-deployer:0.15.0 list
```
...
docker run -it --rm \
  -v "$(pwd):/app/config" \
  api3/airnode-deployer:0.15.0 list --cloud-providers aws
```
...
docker run -it --rm \
  -v "$(pwd):/app/config" \
  api3/airnode-deployer:0.15.0 info aws2c6ef2b3
```
...
docker run -it --rm \
  -v "$(pwd):/app/config" \
  api3/airnode-deployer:0.15.0 rollback aws2c6ef2b3 3580a278
```
...
docker run -it --rm \
  -v "$(pwd):/app/config" \
  api3/airnode-deployer:0.15.0 fetch-files aws2c6ef2b3
```
...
docker run -it --rm \
  -v "$(pwd):/app/config" \
  api3/airnode-deployer:0.15.0 remove aws2c6ef2b3
```
...
docker run -it --rm \
  -v "$(pwd):/app/config" \
  api3/airnode-deployer:0.15.0 remove-with-receipt
```
