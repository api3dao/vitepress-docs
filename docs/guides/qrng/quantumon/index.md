...
[Quantumon]() - Quantum Monsters are a collection of AI generated monsters that are minted to users with the help of [API3's QRNG]() and [DALL-E AI](). It utilises the power of [QRNG]() to generate unique random NFTs.
...
- `endpointIdUint256`: When sending a request to the airnode you need to specify the endpointId. You can think of the `endpointId` as the path in a url. For example if the URL is `www.google.com/images` then `www.google.com` would be the airnode and `/images` would be the `endpointId`.
...
:::tip Note:
In solidity `string(abi.encodePacked(StringA,StringB))` returns the concatenation of `stringA` and `stringB`.
:::
...
:::warning Note:
`generateQuantumon()` has the `onlyAirnodeRrpmodifier` which means that only the `AirnodeRrp`contract can call this function.
:::
...
In this guide, you learnt how Quantumon utilises QRNG to mint Quantumon Monsters. You also learnt how to request data from a QRNG Airnode and how to use the data to mint an NFT.
